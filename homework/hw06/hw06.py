    ...
    def vend(self):
        if self.stock == 0:
            ...
            if change == 0:
                ...
                return (
                    "Here is your " + self.product + " and $" + str(change) + " change."
                )

    def add_funds(self, funds):
        if self.stock == 0:
            return (
                "Nothing left to vend. Please restock. Here is your $"
                + str(funds)
                + "."
            )
        else:
...
class VirFib:
    ...
    def next(self):
        "*** YOUR CODE HERE ***"
        res = VirFib()
        res.pre_value = self.value
        if self.value != 0:
            res.value = self.pre_value + self.value
        else:
            res.value = 1
        return res
